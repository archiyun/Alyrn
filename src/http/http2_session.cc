...
int Http2Session::OnHeaderCb(nghttp2_session*,
  ...
  if      (k == ":method") {
    if      (v == "GET")    st.request.set_method(Method::Get);
    else if (v == "POST")   st.request.set_method(Method::Post);
    else if (v == "PUT")    st.request.set_method(Method::Put);
    else if (v == "DELETE") st.request.set_method(Method::Delete);
    else if (v == "PATCH")  st.request.set_method(Method::Patch);
    else if (v == "HEAD")   st.request.set_method(Method::Head);
    else if (v == "OPTIONS") st.request.set_method(Method::Options);
    st.request.set_version(Version::Http20);
  } else if (k == ":path") {
    const auto q = v.find('?');
    if (q == std::string_view::npos) {
      st.request.set_path(std::string(v));
    } else {
      st.request.set_path(std::string(v.substr(0, q)));
      st.request.set_query(std::string(v.substr(q + 1)));
    }
  } else if (k == ":authority") {
...
int Http2Session::OnFrameRecvCb(nghttp2_session*,
  ...
  if (!st.body_buf.empty())
    st.request.set_body(std::move(st.body_buf));
