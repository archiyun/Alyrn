  ...
  while (state_ != ParseState::GotAll) {
    if (state_ == ParseState::ExpectRequestLine ||
      ...
      if (crlf == nullptr) {
        ...
        const std::size_t pending = buf.readable_bytes();
        if (state_ == ParseState::ExpectRequestLine) {
      ...
      if (buf.readable_bytes() < body_bytes_expected_) return ParseStatus::Continue;

      request_.set_body(buf.RetrieveAsString(body_bytes_expected_));
      request_.set_receive_time(ts);
      state_ = ParseState::GotAll;
    }
  }
