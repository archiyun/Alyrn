...
static void SetTimerfd(int timerfd, runtime::time::Timestamp expiration) {
    ...
    int64_t us = static_cast<int64_t>(
        TimeDifference(expiration, Timestamp::Now()) * 1e6);
    if (us < 100) us = 100; // 至少 100us，避免立即重复触发
...
static void ReadTimerfd(int timerfd) {
    uint64_t how_many;
    ::read(timerfd, &how_many, sizeof(how_many)); // 清除可读状态
}
...
TimerQueue::TimerQueue(EventLoop* loop)
    ...
    timerfd_channel_->SetReadCallback([this](runtime::time::Timestamp) { HandleRead(); });
...
TimerQueue::~TimerQueue() {
    ...
    ::close(timerfd_);
    for (auto& [ts, timer] : timers_) delete timer;
}

TimerId TimerQueue::AddTimer(Callback cb, runtime::time::Timestamp when, double interval) {
    Timer* t = new Timer(std::move(cb), when, interval);
    // 必须在 loop 线程里操作 timers_（无锁）
    loop_->RunInLoop([this, t] {
        bool earliest_changed = timers_.empty() ||
                                t->Expiration() < timers_.begin()->first;
