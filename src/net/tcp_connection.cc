...
void TcpConnection::set_edge_triggered(bool et) {
  channel_->set_edge_triggered(et);
}
...
void TcpConnection::SendInLoop(const void* data, std::size_t len) {
  ...
  if (static_cast<std::size_t>(nwrote) < len) {
    const std::size_t old_len = output_buffer_.readable_bytes();
    output_buffer_.Append(static_cast<const char*>(data) + nwrote,len - nwrote);
    const std::size_t new_len = output_buffer_.readable_bytes();
...
void TcpConnection::Shutdown() {
  if (state_ == TCPState::kConnected) {
    set_state(TCPState::kDisconnecting);
    auto self = shared_from_this();
