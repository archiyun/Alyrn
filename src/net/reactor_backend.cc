...
void ReactorServer::Start() {
  ...
  server_.set_message_callback([this](const TcpConnection::TcpConnectionPtr& c, Buffer& b,
...
void ReactorServer::OnConnection(const TcpConnection::TcpConnectionPtr& conn) {
  if (conn->Connected()) {
    ...
    connection_callback_(std::any_cast<const std::shared_ptr<ReactorConn>&>(conn->context()));
  }
}

void ReactorServer::OnMessage(const TcpConnection::TcpConnectionPtr& conn, Buffer& buf,
                              vexo::time::Timestamp ts) {
  if (!message_callback_) return;
  message_callback_(std::any_cast<const std::shared_ptr<ReactorConn>&>(conn->context()), buf, ts);
}

std::unique_ptr<IServer> MakeReactorServer(vexo::io::Backend backend, EventLoop* loop,
                                           const InetAddress& addr, std::string name) {
  switch (backend) {
    case vexo::io::Backend::kReactor:
      return std::make_unique<ReactorServer>(loop, addr, std::move(name));
    case vexo::io::Backend::kLuring:
      LOG_ERROR() << "reactor server: kLuring is not available; using reactor adapter";
      return std::make_unique<ReactorServer>(loop, addr, std::move(name));
  }
  return nullptr;
}
