...
void UpstreamRequest::ConnectToWithPool(std::shared_ptr<UpstreamPeer> peer,
  ...
  AttachCallbacks();
  // The pooled connection is already established, so TcpClient::SetMessageCallback
  ...
  conn.SetCloseCallback([weak_self](const TcpConnectionPtr& up) {
...
ProxyPass::Forward(const TcpConnectionPtr& client_conn,
  ...
  auto req = std::make_shared<UpstreamRequest>(
      ...
      cb, /*max_retries=*/2, request.GetMethod());
  req->Start();
  return req;
}
