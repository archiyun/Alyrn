...
void Listener::HandleRead() {
  ALYRN_DCHECK(loop_->IsInLoopThread(), "Listener::HandleRead called from wrong thread");
  if (pending_accept_ != nullptr) {
...
void Listener::HandleError() {
  ALYRN_DCHECK(loop_->IsInLoopThread(), "Listener::HandleError called from wrong thread");
  if (pending_accept_ != nullptr) {
...
void Listener::CompleteAccept(Result<Stream> result) {
  ALYRN_DCHECK(loop_->IsInLoopThread(), "Listener::CompleteAccept called from wrong thread");
