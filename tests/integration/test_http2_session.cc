...
TEST(Http2SessionIntegrationTest, MapsRequestPseudoHeadersAndBody) {
  ...
  server = std::make_unique<runtime::http::Http2Session>(
      ...
      [&](runtime::http::HttpRequest& req, runtime::http::HttpResponse& resp,
        ...
        ++dispatch_count;
        EXPECT_EQ(req.method(), runtime::http::Method::Options);
        EXPECT_EQ(req.version(), runtime::http::Version::Http20);
        EXPECT_EQ(req.path(), "/api/items");
        EXPECT_EQ(req.query(), "debug=1");
        EXPECT_EQ(req.header("host"), "example.test");
        EXPECT_EQ(req.header("x-client"), "h2-test");
        EXPECT_EQ(req.body(), "payload-body");

        resp.set_status_code(runtime::http::StatusCode::Created);
...
TEST(Http2SessionIntegrationTest, CompletesEmptyBodyResponsesWithoutDataProvider) {
  ...
  server = std::make_unique<runtime::http::Http2Session>(
      ...
      [&](runtime::http::HttpRequest&, runtime::http::HttpResponse& resp,
          std::shared_ptr<runtime::net::TcpConnection>, int32_t stream_id) {
        resp.set_status_code(runtime::http::StatusCode::NoContent);
