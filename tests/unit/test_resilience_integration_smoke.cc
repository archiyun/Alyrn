...
Outcome Dispatch(runtime::gateway::RateLimiter* rl,
                 runtime::gateway::CircuitBreaker* cb,
...
bool TestNormalFlowPassesThrough() {
  runtime::gateway::RateLimiterConfig rl_cfg;
  ...
  runtime::gateway::RateLimiter rl(rl_cfg);

  runtime::gateway::CircuitBreakerConfig cb_cfg;
  runtime::gateway::CircuitBreaker cb(cb_cfg);
...
bool TestFallbackPreRenderedContent() {
  runtime::gateway::FallbackConfig fallback;
  fallback.enabled = true;
  fallback.status_code = runtime::http::StatusCode::ServiceUnavailable;
