...
using alyrn::epoll::RecvSource;
using alyrn::epoll::RecvSourceOptions;
...
bool CheckImmediateReceive() {
  ...
  constexpr std::string_view kPayload = "epoll-immediate";
...
bool CheckPendingReceive() {
  ...
  loop.RunAfter(alyrn::time::Duration::zero(), [sender = fds[1]] {
    constexpr std::string_view kPayload = "epoll-pending";
