...
bool TestConsistentHashSameKeySamePeer() {
  ...
  coropact::gateway::ConsistentHashLB lb(150, "client_ip");

  coropact::gateway::RequestContext ctx{.client_ip = "10.0.0.1"};
...
bool TestConsistentHashDifferentKeysSpread() {
  ...
  for (int i = 0; i < 500; i++) {
    coropact::gateway::RequestContext ctx{.client_ip = "10.0.0." + std::to_string(i)};
    auto peer = lb.Select(*upstream, ctx);
...
bool TestConsistentHashWeightedDistribution() {
  ...
  for (int i = 0; i < 2000; i++) {
    coropact::gateway::RequestContext ctx{.client_ip = "192.168." + std::to_string(i / 256) + "." + std::to_string(i % 256)};
    auto peer = lb.Select(*upstream, ctx);
...
bool TestConsistentHashRingRebuildOnPeerChange() {
  ...
  coropact::gateway::RequestContext ctx{.client_ip = "10.0.0.5"};
...
bool TestConsistentHashHashOnFields() {
  ...
  auto upstream = MakeUpstream("test_ch", {
      {"peer-a:80", 1}, {"peer-b:80", 1}, {"peer-c:80", 1}});
  coropact::gateway::ConsistentHashLB lb(150, "uri");

  coropact::gateway::RequestContext ctx1{.uri = "/users/42"};
  coropact::gateway::RequestContext ctx2{.uri = "/users/42"};  // same URI
  coropact::gateway::RequestContext ctx3{.uri = "/posts/99"};   // different URI
...
bool TestConsistentHashVNodNodesBounds() {
  ...
  coropact::gateway::ConsistentHashLB lb_low(50, "client_ip");
  coropact::gateway::ConsistentHashLB lb_high(500, "client_ip");
...
bool TestConsistentHashHashRingUniqueVirtualNodes() {
  ...
  for (int i = 0; i < 100; i++) {
    coropact::gateway::RequestContext ctx{.client_ip = "172.16." + std::to_string(i / 256) + "." + std::to_string(i % 256)};
    auto peer = lb.Select(*upstream, ctx);
...
bool TestConsistentHashConcurrentSelectWithPeerChanges() {
  ...
  auto lb = std::make_shared<coropact::gateway::ConsistentHashLB>(150, "client_ip");
  ...
  std::unordered_set<coropact::gateway::UpstreamPeer*> valid;
  ...
  for (int t = 0; t < kReaders; ++t) {
    readers.emplace_back([&, t] {
      for (int i = 0; i < kItersPerReader; ++i) {
        coropact::gateway::RequestContext ctx{
...
bool TestMaglevHashSameKeySamePeer() {
  auto upstream = MakeUpstream("test_maglev", {
      {"peer-a:80", 1}, {"peer-b:80", 1}, {"peer-c:80", 1}});
  coropact::gateway::MaglevHashLB lb(257, "client_ip");
...
bool TestMaglevHashExcludesDownPeers() {
  ...
  coropact::gateway::MaglevHashLB lb(257, "client_ip");
  for (int i = 0; i < 200; i++) {
    coropact::gateway::RequestContext ctx{.client_ip = "10.0.1." + std::to_string(i)};
    auto peer = lb.Select(*upstream, ctx);
...
bool TestIPHashSameIPSamePeer() {
  auto upstream = MakeUpstream("test_iph", {
      {"peer-a:80", 1}, {"peer-b:80", 1}, {"peer-c:80", 1}});
  coropact::gateway::IPHashLB lb;

  coropact::gateway::RequestContext ctx{.client_ip = "192.168.1.100"};
  auto first = lb.Select(*upstream, ctx);
  for (int i = 0; i < 100; i++) {
...
bool TestIPHashEmptyIPUsesFallback() {
  ...
  coropact::gateway::RequestContext ctx;  // empty client_ip
  auto peer = lb.Select(*upstream, ctx);
...
bool TestRoundRobinCycles() {
  auto upstream = MakeUpstream("test_rr", {
      {"peer-a:80", 1}, {"peer-b:80", 1}, {"peer-c:80", 1}});
  coropact::gateway::RoundRobinLB lb;
...
bool TestRoundRobinHonorsPassiveFailTimeout() {
  auto upstream = std::make_shared<coropact::gateway::Upstream>(
      coropact::gateway::UpstreamConfig{.name = "test_rr_passive"});
  auto cooling = std::make_shared<coropact::gateway::UpstreamPeer>(
      coropact::gateway::UpstreamPeerConfig{
          .name = "cooling:80",
          .host = "127.0.0.1",
          .port = static_cast<uint16_t>(80),
  ...
  auto healthy = std::make_shared<coropact::gateway::UpstreamPeer>(
      coropact::gateway::UpstreamPeerConfig{
          .name = "healthy:80",
          .host = "127.0.0.1",
          .port = static_cast<uint16_t>(80),
...
bool TestPassivePeerRecoversAfterFailTimeout() {
  coropact::gateway::UpstreamPeer peer(coropact::gateway::UpstreamPeerConfig{
      .name = "passive:80",
      .host = "127.0.0.1",
      .port = static_cast<uint16_t>(80),
      .weight = 4,
...
bool TestWeightedLeastConnectionPicksLowestRatio() {
  ...
  coropact::gateway::WeightedLeastConnectionLB lb;
...
bool TestCreateLoadBalancerAllAlgos() {
  ...
  for (const auto& algo : algos) {
    auto lb = coropact::gateway::CreateLoadBalancer(algo);
    if (!lb) {
  ...
  auto unknown = coropact::gateway::CreateLoadBalancer("bogus");
