...
static void bench_sequential() {
    ...
    // pool
    alyrn::memory::MemoryPool<sizeof(void*), alignof(void*), kCap> pool;
    auto t0 = Clock::now();
...
static void bench_object_pool() {
    ...
    // ObjectPool
    alyrn::memory::ObjectPool<Task, kCap> pool;
    auto t0 = Clock::now();
    for (long long r = 0; r < kRound; ++r) {
        for (std::size_t i = 0; i < kCap; ++i)
...
static void bench_null_mutex() {
    ...
    // NullMutex pool
    alyrn::memory::MemoryPool<sizeof(void*), alignof(void*), kCap,
                                alyrn::memory::NullMutex> pool;
    auto t0 = Clock::now();
    for (long long i = 0; i < kN; ++i) {
        void* p = pool.Allocate();
        sink(p);
