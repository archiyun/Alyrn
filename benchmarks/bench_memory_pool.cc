...
static void bench_sequential() {
    ...
    // pool
    alyrn::detail::MemoryPool<sizeof(void*), alignof(void*), kCap> pool;
    auto t0 = Clock::now();
...
static void bench_object_pool() {
    ...
    // ObjectPool
    alyrn::detail::ObjectPool<Task, kCap> pool;
    auto t0 = Clock::now();
    for (long long r = 0; r < kRound; ++r) {
        for (std::size_t i = 0; i < kCap; ++i)
...
static void bench_null_mutex() {
    ...
    // NullMutex pool
    alyrn::detail::MemoryPool<sizeof(void*), alignof(void*), kCap,
                                      alyrn::detail::NullMutex> pool;
    auto t0 = Clock::now();
    for (long long i = 0; i < kN; ++i) {
        void* p = pool.Allocate();
        sink(p);
