...
[[nodiscard]]
coropact::base::Result<void> set_non_blocking(int fd, bool on = true);

// Sets or clears FD_CLOEXEC on the given fd.
[[nodiscard]]
coropact::base::Result<void> set_close_on_exec(int fd, bool on = true);

// Enables or disables SO_REUSEADDR on the given socket.
[[nodiscard]]
coropact::base::Result<void> set_reuse_addr(int fd, bool on = true);

// Enables or disables SO_REUSEPORT on the given socket.
[[nodiscard]]
coropact::base::Result<void> set_reuse_port(int fd, bool on = true);

// Enables or disables TCP_NODELAY on the given socket.
[[nodiscard]]
coropact::base::Result<void> set_tcp_non_delay(int fd, bool on = true);

// Enables or disables SO_KEEPALIVE on the given socket.
[[nodiscard]]
coropact::base::Result<void> set_keep_alive(int fd, bool on = true);
