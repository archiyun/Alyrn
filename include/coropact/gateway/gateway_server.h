...
template <coropact::io::AsyncListener Listener, UpstreamConnector Connector>
class GatewayServer {
public:
  COROPACT_DELETE_COPY_MOVE(GatewayServer);
  ...
  GatewayServer(Listener& listener, coropact::coro::Scheduler& scheduler, std::string name,
  ...
  void Start() {
    if (started_) return;
    started_ = true;
    coropact::coro::Spawn(scheduler_, AcceptLoop()).Detach();
    if (health_check_enabled_) {
      if constexpr (requires { connector_.SleepFor(std::chrono::milliseconds{1}); }) {
        coropact::coro::Spawn(scheduler_, HealthLoop()).Detach();
      }
    }
  }

  coropact::coro::Task<void> Stop() {
    stopping_ = true;
...
private:
  ...
  coropact::coro::Task<void> AcceptLoop() {
    for (;;) {
      auto accepted = co_await listener_.Accept();
      if (!accepted.has_value()) {
        co_return;
      }
      coropact::coro::Spawn(scheduler_, Session(std::move(*accepted))).Detach();
    }
  }

  coropact::coro::Task<void> Session(Stream stream) {
    ...
    for (;;) {
      ...
      while (parser.GotAll()) {
        coropact::http::HttpRequest& req = parser.CurrentRequest();
  ...
  coropact::coro::Task<bool> HandleParseStatus(Stream& stream, coropact::http::ParseStatus status) {
    if (status == coropact::http::ParseStatus::Continue || status == coropact::http::ParseStatus::GotAll) {
      co_return true;
    }

    auto action = core_.HandleParseError(status);
    co_await coropact::io::WriteAll(stream, Bytes(action.response));
  ...
  coropact::coro::Task<bool> ApplyAction(Stream& stream, const coropact::http::HttpRequest& req,
                                     GatewayCore::Action action,
                                     ProxyForwardBuffers& proxy_buffers) {
    if (action.kind == GatewayActionKind::Send) {
      auto written = co_await coropact::io::WriteAll(stream, Bytes(action.response));
    ...
    if (result.status == ProxyForwardStatus::kNoPeer) {
      ...
      co_await coropact::io::WriteAll(stream, Bytes(fallback.response));
      co_return true;
    }
  ...
  template <coropact::io::AsyncReadStream AnyStream>
  ...
  coropact::coro::Task<void> HealthLoop() {
  ...
  coropact::coro::Task<void> HealthCheckRound() {
  ...
  coropact::coro::Task<bool> ProbePeer(UpstreamPeer& peer) {
    ...
    const std::string request = "GET " + health_check_cfg_.path +
                                ...
                                "Host: " +
                                peer.host_port() + "\r\nConnection: close\r\n\r\n";
    auto written = co_await coropact::io::WriteAll(stream, Bytes(request));
  ...
  Listener& listener_;
  coropact::coro::Scheduler& scheduler_;
