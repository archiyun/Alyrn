...
class LoopShutdownParticipant : public ds::ListNode<LoopShutdownParticipant> {
public:
  ...
  COROPACT_DELETE_COPY_MOVE(LoopShutdownParticipant);
...
// adapter, and a shared module may only be extracted once both backends have
// proven the contract is genuinely backend-neutral.
class LoopShutdownRegistry final {
public:
  LoopShutdownRegistry() noexcept = default;

  COROPACT_DELETE_COPY_MOVE(LoopShutdownRegistry);
...
private:
  ds::IntrusiveList<LoopShutdownParticipant> participants_;
};
