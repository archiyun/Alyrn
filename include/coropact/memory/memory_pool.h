...
class MemoryPool {
 ...
 public:
  ...
  COROPACT_DELETE_COPY_MOVE(MemoryPool);
  ...
  void* Allocate() noexcept {
    ...
    COROPACT_CHECK(!SlotAllocated(slot) && !SlotReleasing(slot),
  ...
  void Deallocate(void* ptr) noexcept {
    ...
    COROPACT_CHECK(Owns(ptr), "MemoryPool::Deallocate: foreign pointer");
 ...
 private:
  ...
  void MarkSlotAllocated(std::size_t slot) noexcept {
    const std::size_t word = slot / kStateWordBits;
    const std::size_t bit = slot % kStateWordBits;
    COROPACT_CHECK(!SlotReleasing(slot), "MemoryPool::Allocate: releasing slot reused");
  ...
  void ClaimDeallocationLocked(void* ptr) noexcept {
    const std::size_t slot = SlotIndex(ptr);
    COROPACT_CHECK(SlotAllocated(slot), "MemoryPool::Deallocate: double free");
    MarkSlotReleasing(slot);
  }

  void FinishDeallocationLocked(void* ptr) noexcept {
    const std::size_t slot = SlotIndex(ptr);
    COROPACT_CHECK(SlotReleasing(slot), "MemoryPool::Deallocate: missing release claim");
    MarkSlotFree(slot);
  ...
  void ClaimDeallocation(void* ptr) noexcept {
    COROPACT_CHECK(Owns(ptr), "MemoryPool::ClaimDeallocation: foreign pointer");
    std::lock_guard<MutexPolicy> lock{mutex_};
    ClaimDeallocationLocked(ptr);
  }

  void FinishDeallocation(void* ptr) noexcept {
    COROPACT_CHECK(Owns(ptr), "MemoryPool::FinishDeallocation: foreign pointer");
    std::lock_guard<MutexPolicy> lock{mutex_};
    FinishDeallocationLocked(ptr);
  }
