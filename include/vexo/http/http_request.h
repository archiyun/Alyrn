...
class HttpRequest {
 public:
  ...
  void set_receive_time(vexo::time::Timestamp ts) { receive_time_ = ts; }
  vexo::time::Timestamp receive_time() const { return receive_time_; }
 ...
 private:
  ...
  vexo::memory::Pool::Ptr pool_;
  std::unique_ptr<vexo::memory::PoolResource> res_;
  ...
  vexo::time::Timestamp receive_time_;
};
