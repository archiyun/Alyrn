...
class LRUCache {
public:
  VEXO_DELETE_COPY_MOVE(LRUCache);
  ...
  void Put(const Key& key, Value value, vexo::time::Timestamp expire_at) {
  ...
  void Put(const Key& key, Value value, double ttl_seconds) {
    PutInternal(key, std::move(value),
                vexo::time::AddTime(vexo::time::Timestamp::Now(), ttl_seconds));
  }
...
private:
  struct Entry {
    Key key;
    Value value;
    std::optional<vexo::time::Timestamp> expire_at;
  };
  ...
  [[nodiscard]] bool IsExpired(ListIt entry) const {
    return entry->expire_at.has_value() && vexo::time::Timestamp::Now() >= *entry->expire_at;
  }
  ...
  void PutInternal(const Key& key, Value value, std::optional<vexo::time::Timestamp> expire_at) {
