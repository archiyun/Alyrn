...
public:
  std::shared_ptr<UpstreamPeer> Select(Upstream& upstream, const RequestContext ctx = {}) override {
    ...
    uint32_t hash = vexo::ds::MurmurHash3(key);
...
public:
  ...
  std::shared_ptr<UpstreamPeer> Select(Upstream& upstream, const RequestContext ctx = {}) override {
    ...
    if (ring && ring->fingerprint == fp) {
      return Lookup(*ring, vexo::ds::MurmurHash64(HashKey(ctx)));
    }
  ...
  std::shared_ptr<const HashRing> BuildRing(
    ...
    for (const auto& peer : peers) {
      ...
      for (int i = 0; i < vnode_cnt; i++) {
        ...
        uint64_t hash = vexo::ds::MurmurHash64(vnode_key);
...
public:
  ...
  std::shared_ptr<UpstreamPeer> Select(Upstream& upstream, const RequestContext ctx = {} ) override {
    ...
    if (table && table->fingerprint == fp) {
      return Lookup(*table, vexo::ds::MurmurHash64(HashKey(ctx)));
    }
  ...
  std::shared_ptr<const LookupTable> BuildTable(
    ...
    for (const auto& peer : peers) {
      ...
      for (int replica = 0; replica < weight; ++replica) {
        ...
        const uint64_t h1 = vexo::ds::MurmurHash64(id, 0);
        const uint64_t h2 = vexo::ds::MurmurHash64(id, 0x9e3779b9U);
