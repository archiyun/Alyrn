...
#define ALYRN_DELETE_MOVE(cls) \
  cls(cls&&) = delete;        \
  cls& operator=(cls&&) = delete

// Deletes both copy and move operations for cls.
#define ALYRN_DELETE_COPY_MOVE(cls) \
  ALYRN_DELETE_COPY(cls);           \
  ALYRN_DELETE_MOVE(cls)
...
#define ALYRN_DISABLE_COPY_ALLOW_MOVE(cls) \
  ALYRN_DELETE_COPY(cls);                  \
  cls(cls&&) = default;                   \
