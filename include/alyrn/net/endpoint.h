...
public:
  ...
  explicit Endpoint(const sockaddr* address, socklen_t length) noexcept {
    ALYRN_CHECK(address != nullptr, "Endpoint: address must not be null");

    switch (address->sa_family) {
      case AF_INET:
        ALYRN_CHECK(length >= static_cast<socklen_t>(sizeof(sockaddr_in)),
        ...
        return;
      case AF_INET6:
        ALYRN_CHECK(length >= static_cast<socklen_t>(sizeof(sockaddr_in6)),
        ...
        return;
      default:
        ALYRN_CHECK(false, "Endpoint: unsupported address family");
    }
  }
  ...
  std::string ToIp() const {
    ...
    switch (NativeFamily()) {
      ...
      case AF_INET6:
        ...
        break;
      default:
        ALYRN_CHECK(false, "Endpoint::ToIp: unsupported address family");
    }

    ALYRN_CHECK(result != nullptr, "Endpoint::ToIp: inet_ntop failed");
    return result;
  }
