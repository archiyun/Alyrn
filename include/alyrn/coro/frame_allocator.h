...
inline std::uint64_t PackFrameMetadata(std::size_t bytes, std::size_t alignment) {
  ALYRN_CHECK(bytes <= kFrameMetadataBytesMask,
                 "coroutine frame metadata cannot encode this allocation size");
  ALYRN_CHECK(alignment != 0 && std::has_single_bit(alignment),
  ...
  ALYRN_CHECK(exponent <= kFrameMetadataMaxAlignmentExponent,
...
public:
  ...
  ALYRN_DELETE_COPY_MOVE(CoroFramePoolResource);
...
private:
  ...
  static void RegisterSlab(SlabHeader* slab) {
    ...
    ALYRN_CHECK((addr & (kChunkBytes - 1)) == 0, "frame slab must be chunk-aligned");
    std::size_t l1 = 0;
    std::size_t l2 = 0;
    SlabMapIndex(addr, &l1, &l2);
    ALYRN_CHECK(l1 < kL1Size, "frame slab address exceeds the slab map");
    ...
    ALYRN_CHECK(directory->entries[l2].compare_exchange_strong(
  ...
  void AddChunk(SizeClass& size_class, std::size_t index) {
    ...
    ALYRN_CHECK(slot_count > 0, "frame size class does not fit in a slab");

    void* allocation = upstream_->allocate(kChunkBytes, kChunkBytes);
    ALYRN_CHECK((reinterpret_cast<std::uintptr_t>(allocation) & (kChunkBytes - 1)) == 0,
...
public:
  ALYRN_DELETE_COPY(FrameAllocatorScope);
