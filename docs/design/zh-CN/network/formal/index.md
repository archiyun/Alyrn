...
这些模型不是运行时代码的逐行翻译，而是对 Alyrn 可观察语义的有限状态验证：
提交、完成、取消、协程恢复与资源释放分别在何处线性化。`await_suspend()` 返回 `false`
的立即路径不产生 `ResumeWork`；它由当前协程内联执行 `await_resume()`，与真正的
scheduler 恢复分开建模。
...
7. `async_stream_backend_refinement.tla`：Epoll 与 luring 对抽象模型的映射。
...
| `async_stream_backend_refinement.tla` | `ObsEpoll` / `ObsUring` 如何隐藏 readiness、SQE/CQE step 并映射到同一逻辑 trace |
...
| `accept_source_refinement.tla` | Epoll accept drain 与 luring multishot accept 的逻辑 source 语义 |
