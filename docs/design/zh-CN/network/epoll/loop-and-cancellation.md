...
```text
epoll::Loop                  公开的单线程 dispatcher / Scheduler
epoll::Stream              socket 的 read/write/close 适配器
epoll::Listener            accept 与 AcceptSource 适配器
epoll::Connector           connect / SleepFor 适配器
epoll::RecvSource          readiness 驱动的多事件 receive source

epoll::detail::Channel            一个 fd 的 interest、事件与回调
epoll::detail::Poller             poller seam；默认实现是 EPollPoller
epoll::detail::TimerQueue         owner-loop timer
epoll::detail::LoopShutdownRegistry
                                  loop stop 时通知 loop-owned 资源
```

公开 seam 很小：业务代码只持有 `Loop`、stream、listener 或 connector。
`Channel`、`epoll_ctl`、pending awaiter 指针和关闭注册表都留在 Epoll 的实现中。
...
每个 stream 最多同时拥有一个 pending read 和一个 pending write。这是当前 Epoll
...
Epoll 使用普通 nonblocking syscall：
...
到 `await_resume()` 返回时，Epoll 后端已经不会再访问 borrowed buffer。这里没有 luring
...
Epoll 不允许多个线程直接执行 `CompleteRead()` 或 `CompleteWrite()`。跨线程 stop 先通过
...
槽位在结果固定后、恢复 continuation 前才释放；`epoll_listener_smoke_test` 的
...
事件队列或 buffer slot 到达 high-water 不是 `ENOBUFS` 终止错误。Epoll 会将当前
...
其中 `resource_close_cancel.tla` 是跨后端的更强模型。Epoll 对 borrowed read 的实现较简单，
...
最后再读 event source：`src/epoll/listener.cc` 的 `AcceptSource` 与
`src/epoll/recv_source.cc`。这样不会一开始就被 multishot/lease 状态机淹没。
