---
name: runtime-epoll-maintenance
description: Maintain the alyrn/epoll Linux epoll backend, Loop, Channel, poller, and timers. Use for epoll-layer patches, reviews, lifetime fixes, and threading changes.
---

# alyrn/epoll Maintenance

Read `docs/SUBSYSTEMS.md` before editing.

## Purpose

Own Linux epoll mechanics: Loop dispatch, Poller registration, Channel
event delivery, epoll socket ownership, callback scheduling, and timerfd
integration. Shared address and socket primitives remain in `alyrn/net`.
...
- An epoll stream owns its transport state and is destroyed on its owning loop.
...
- Epoll code contains no gateway-specific naming or policy.
...
- `epoll_loop_smoke_test`
- `epoll_stream_smoke_test`
- `epoll_listener_smoke_test`
- `net_move_smoke_test` for detached Channel and Socket ownership transfer
- `timer_tree_smoke_test`
- `timer_index_smoke_test`
- `io_buffer_smoke_test` for the public `alyrn::io::Buffer`
