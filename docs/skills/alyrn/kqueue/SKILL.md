...
description: Maintain the alyrn/kqueue BSD/Darwin readiness backend, Loop, Post, Channel, poller, timers, and master-slave worker handoff. Use for kqueue-layer patches, reviews, lifetime fixes, and threading changes.
---

# alyrn/kqueue Maintenance
...
- Native: `-DALYRN_ENABLE_KQUEUE=ON` on FreeBSD/NetBSD/OpenBSD/Darwin
  (worker-group and Runtime smoke).
- Linux shim: `-DALYRN_ENABLE_KQUEUE_SHIM_TESTS=ON` (oneshot state machine
