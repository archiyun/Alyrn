...
- `C=5000` 时 luring 为 `558.9k RPS / 10.91 ms P99`，raw liburing 为 `553.6k RPS / 13.56 ms P99`。约 1% 的吞吐差异不具显著性；luring 在本轮的 P99 更低。相对 Reactor，luring 吞吐高 `13.6%`，P99 低 `1.68 ms`。
- Reactor 在 `C=100` 与 `C=1000` 的 P99 分别为 `0.63 ms` 和 `2.34 ms`，低于 luring；这是 epoll readiness 路径在这项小型固定响应 workload 下的低延迟表现，不应据此推导一般业务结论。
...
- raw liburing：当前 checkout 构建的 `examples/luring/demo_bench_http_liburing`；每 worker 维持 accept depth 4，并在 SQE 暂时耗尽时 FIFO 重试，而不是关闭连接。
...
| 并发 | Reactor | luring | raw liburing | Asio | libuv | libevent | libev |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
