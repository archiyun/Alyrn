...
Reactor 使用 epoll 监听 socket readiness，再通过非阻塞 `accept`、`recv` 和 `send` drain 出逻辑事件。`REACTOR_TRIGGER_MODE` 只对 Reactor 生效：

- `et`：edge-triggered readiness；
- `lt`：level-triggered readiness。

因此本次分别测试了 Reactor ET 和 Reactor LT，用于观察 readiness 触发策略对吞吐与尾延迟的影响。
...
本次 luring benchmark 使用 demo 的默认 single-shot accept，accept depth 为 4；协程帧池开启。
...
## 结果一：Reactor ET 对 luring

此批次先运行 luring，再运行 Reactor ET。luring 列是 luring 的一次独立参考运行，不是 luring 的 ET 模式。

| 并发 | luring RPS / P99 | Reactor ET RPS / P99 |
| ---: | ---: | ---: |
...
## 结果二：Reactor LT 对 luring

此批次先运行 luring，再运行 Reactor LT。这里的 luring 列仍然只是 luring 的另一次独立参考运行，不能称为 luring LT。

| 并发 | luring RPS / P99 | Reactor LT RPS / P99 |
| ---: | ---: | ---: |
...
| ET / Reactor | 259.3% | 187.3 MiB |
| LT / luring | 235.0% | 163.6 MiB |
| LT / Reactor | 250.3% | 184.9 MiB |

两批次所有档位均为 0 connect/read/write error、0 timeout、0 non-2xx response。

## 结论边界

- luring 在 C≥2000 的两批次中通常比 Reactor ET/LT 有更高吞吐；C=1000 附近两者接近，Reactor P99 略有优势。
- C≥2000 时 luring 的 P99 通常也低于 Reactor；这反映的是本次固定 HTTP workload 下的完成路径差异，不代表所有业务负载都成立。
- luring 两次结果之间存在正常运行波动，不能从两次数据推导“luring ET/LT 差异”。正确的对比关系只有：`luring vs Reactor ET` 和 `luring vs Reactor LT`。
