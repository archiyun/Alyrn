...
- The dependency direction is one-way from lower primitives to concrete
  ...
  `coropact_io` target.
- `net::SegmentedBuffer` owns backend-neutral segmented byte storage;
  `net::WritePart` and accept-source admission state remain network primitives.
  `io::Buffer` is the public zero-cost spelling for `net::SegmentedBuffer`.
  The `io` facade may compose these lower modules but does not own their
  implementations.
