...
tag 在编译期选择实现，而不是用运行时 enum 抹平配置。需要手动控制 loop、timer、mailbox、资源注册
或能力选择的用户继续直接组合 `EventLoop` 或 `LUringLoop` 及其公开 adapter。

不新增运行时 `Backend(...)` 配置、类型擦除的通用 stream handler、额外 Builder tuning 方法或 C++
main 宏。未来只有在两个后端长期拥有相同、已验证且无需削弱原生能力的应用级语义时，才重新评估
这些便利层。
